...
.. automodule:: pyplc.core
   ...
   Программа логики контроллера должна содержать один экземпляр этого класса. Как правило создается в служебном модуле :py:mod:`pyplc.config` (но можно и явно самостоятельно создать). 
...
.. automodule:: pyplc.sfc
   ...
   Далее в коде на месте, где должен быть переход на следующий шаг (следующий цикл логики) используем ключевое слово `yield`, или конструкцию `yield from`
   ...
   методов-генераторов можно запустить параллельно основной логике с помощью :py:meth:`~pyplc.sfc.SFC.exec`, и остановить в любое время.

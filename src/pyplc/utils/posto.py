...
class Subscriber(TCPClient):
    class __State():
        ...
        # используется POSTO при оформлении подписки
    ...
    def connected(self):
        print(
            f'Subscriber is online. Pending {len(self.unsubscribed)} subsciptions')

    def disconnected(self):
        print(
            f'Subscriber is offline. Gone {len(self.subscriptions)-len(self.unsubscribed)} subscriptions')
        self.online = False
